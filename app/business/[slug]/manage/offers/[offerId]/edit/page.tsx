...
import { getOwnerOfferById } from "@/lib/business-offers/queries";
...
export default async function EditOfferPage({ params }: EditOfferPageProps) {
  ...
  if (!user) redirect(`/login?next=/business/${slug}/manage/offers/${offerId}/edit`);
  ...
  const offer = await getOwnerOfferById(client, offerId);
  if (!offer || offer.businessId !== business.id) notFound();
