...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    if (!listing || listing.listingType !== "transfer") {
      return {
    ...
    return {
      title: `${listing.title} — Переводы`,
      description,
      alternates: { canonical: `${siteUrl}/transfers/${listing.id}` },
      openGraph: {
        title: listing.title,
        description,
        url: `${siteUrl}/transfers/${listing.id}`,
...
export default async function TransferDetailPage({ params }: PageProps) {
  ...
  try {
    ...
    loadError =
      err instanceof Error ? err.message : "Не удалось загрузить перевод";
  }

  if (loadError) {
    return <ErrorState detail={loadError} message="Перевод недоступен" />;
  }
      ...
      return (
        <div className="mx-auto max-w-lg py-16 text-center">
          ...
          <p className="mt-2 text-slate-600">
            Он снят с публикации или отклонён модерацией.
          </p>
          <Link
  ...
  if (transfer?.feePercent != null) feeParts.push(`${transfer.feePercent}%`);
  if (transfer?.feeFixedUsd != null) {
    feeParts.push(formatMoney(transfer.feeFixedUsd, listing.priceCurrency));
  }
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          ...
          <p className="text-3xl font-bold text-slate-900">
            {feeParts.length ? feeParts.join(" + ") : "Комиссия не указана"}
          </p>
          ...
          <dl className="grid gap-3 text-sm sm:grid-cols-2">
            ...
            {transfer?.processingDays != null && (
              <div>
                <dt className="text-slate-500">Срок</dt>
                <dd className="font-medium text-slate-900">
                  {transfer.processingDays} дн.
                </dd>
              </div>
            )}
            {(transfer?.minAmountUsd != null ||
              transfer?.maxAmountUsd != null) && (
              <div>
                <dt className="text-slate-500">Сумма</dt>
                <dd className="font-medium text-slate-900">
                  {[
                    transfer.minAmountUsd != null &&
                      `от ${formatMoney(transfer.minAmountUsd, listing.priceCurrency)}`,
                    transfer.maxAmountUsd != null &&
                      `до ${formatMoney(transfer.maxAmountUsd, listing.priceCurrency)}`,
                  ]
                    .filter(Boolean)
                    .join(" · ")}
                </dd>
