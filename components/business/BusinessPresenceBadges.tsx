import { cn } from "@/lib/utils";
import {
  hasGoogleMapsPresence,
  normalizeGoogleRating,
  resolveInstagramUrl,
  type BusinessPresence,
} from "@/lib/business/presence";

function GoogleGlyph({ className }: { className?: string }) {
  return (
    <svg
      aria-hidden="true"
      className={className}
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
        fill="#4285F4"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        fill="#FBBC05"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill="#EA4335"
      />
    </svg>
  );
}

function InstagramGlyph({ className }: { className?: string }) {
  return (
    <svg
      aria-hidden="true"
      className={className}
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M7 2h10a5 5 0 0 1 5 5v10a5 5 0 0 1-5 5H7a5 5 0 0 1-5-5V7a5 5 0 0 1 5-5zm5 5.2A4.8 4.8 0 1 0 16.8 12 4.8 4.8 0 0 0 12 7.2zm6.1-.95a1.15 1.15 0 1 0 1.15 1.15 1.15 1.15 0 0 0-1.15-1.15z"
        fill="#E4405F"
      />
    </svg>
  );
}

type BusinessPresenceBadgesProps = {
  presence: BusinessPresence;
  className?: string;
};

export function BusinessPresenceBadges({
  presence,
  className,
}: BusinessPresenceBadgesProps) {
  const instagram = resolveInstagramUrl(presence);
  const onGoogle = hasGoogleMapsPresence(presence);
  const rating = normalizeGoogleRating(presence.googleRating);

  if (!instagram && !onGoogle) return null;

  return (
    <div
      className={cn(
        "mt-1.5 flex flex-wrap items-center gap-1.5",
        className,
      )}
    >
      {onGoogle ? (
        <span
          className={cn(
            "inline-flex items-center gap-1 rounded-md border border-slate-200 bg-white px-1.5 py-0.5 text-xs font-medium text-slate-700",
            rating != null && "pr-2",
          )}
          title={
            rating != null
              ? `Google ${rating.toFixed(1)}`
              : "Есть на Google Maps"
          }
        >
          <GoogleGlyph className="size-3.5 shrink-0" />
          {rating != null ? (
            <span className="tabular-nums">{rating.toFixed(1)}</span>
          ) : null}
        </span>
      ) : null}

      {instagram ? (
        <span
          className="inline-flex size-6 items-center justify-center rounded-md border border-slate-200 bg-white"
          title="Instagram"
        >
          <InstagramGlyph className="size-3.5" />
        </span>
      ) : null}
    </div>
  );
}
