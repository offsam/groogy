...
function flagChips(church: Church) {
  const flags = church.presenceFlags;
...
export function ChurchContactsCard({
  church,
  isAuthenticated = false,
  initiallyRevealed = false,
}: ChurchContactsCardProps) {
  const hasServerContacts = Boolean(
    church.phone?.trim() ||
      church.email?.trim() ||
      church.website?.trim() ||
      church.instagramUrl?.trim() ||
      church.telegramUrl?.trim() ||
      church.contactLinks.length > 0,
  );
  ...
  const previewChips = flagChips(church);
  const hasAnyFlag = previewChips.length > 0;

  const items = buildItems({
    phone: fetched?.phone ?? church.phone,
    email: fetched?.email ?? church.email,
    website: fetched?.website ?? church.website,
    instagramUrl: fetched?.instagramUrl ?? church.instagramUrl,
    telegramUrl: fetched?.telegramUrl ?? church.telegramUrl,
    contactLinks: fetched?.contactLinks ?? church.contactLinks,
  });
  ...
  async function loadContacts() {
    ...
    try {
      const res = await fetch(
        `/api/church/${encodeURIComponent(church.slug)}/contacts`,
      );
      if (res.status === 401) {
        setAuthOpen(true);
        return;
  ...
  useEffect(() => {
    ...
    if (window.location.hash !== "#church-contacts") return;
    void loadContacts();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isAuthenticated, revealed, church.slug]);
      ...
      <QuickAuthModal
        nextPath={
          typeof window !== "undefined"
            ? `${window.location.pathname}${window.location.search || ""}#church-contacts`
            : `/churches/${church.slug}#church-contacts`
        }
        open={authOpen}
