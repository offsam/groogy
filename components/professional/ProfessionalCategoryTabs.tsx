...
function hrefFor(
  ...
  if (target === "all") {
    q.set("view", "all");
    return `/professionals?${q.toString()}`;
  }
...
/**
 * Sphere icons on the professionals overview — same pattern as businesses.
 * Each opens a list (Все / sphere); overview itself shows no cards.
 */
export function ProfessionalCategoryTabs({
  categories,
  categoryCounts,
  totalCount,
  hubParam,
  ...
  return (
    <nav aria-label="Сферы специалистов">
      <ul className="grid grid-cols-4 gap-x-2 gap-y-4 sm:grid-cols-5 sm:gap-x-3">
        ...
        {items.map((cat) => {
          const n = categoryCounts[cat.slug] ?? 0;
          if (n <= 0) return null;
          return (
            <li key={cat.id}>
              <Link
              ...
              >
                <span className="flex size-14 items-center justify-center rounded-full bg-slate-100 text-slate-700 transition group-hover:bg-brand-blue/10 group-hover:text-brand-blue sm:size-16">
                  <CategoryIcon className="size-6 sm:size-7" slug={cat.slug} />
                </span>
                <span className="max-w-[4.5rem] text-[11px] leading-tight sm:max-w-[5.5rem] sm:text-xs">
