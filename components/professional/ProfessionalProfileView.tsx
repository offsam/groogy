...
type ProfessionalProfileViewProps = {
  ...
  preview?: boolean;
  /**
   * Queue/import preview: amber admin strip where live AdminLensBar sits.
   */
...
export function ProfessionalProfileView({
  ...
  // One block; «Показать полностью» truncates inside DescriptionWithOriginal.
  const aboutText = fullAbout || shortAbout || null;
          ...
          <div className="space-y-1 text-sm text-slate-500">
            ...
            <ExternalRatingChips
              items={businessExternalRatingItems({
                googleRating:
                  professional.googleRating != null &&
                  Number(professional.googleRating) > 0
                    ? professional.googleRating
                    : professional.employerBusinessGoogleRating,
                googleReviewsCount:
                  professional.googleRating != null &&
                  Number(professional.googleRating) > 0
                    ? professional.googleReviewsCount
                    : professional.employerBusinessGoogleReviewsCount,
                yelpRating: professional.yelpRating,
                yelpReviewsCount: professional.yelpReviewsCount,
                trustpilotRating: professional.trustpilotRating,
                trustpilotReviewsCount: professional.trustpilotReviewsCount,
              })}
            />
          </div>
          {bookHref ? (
