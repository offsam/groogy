...
export const UPDATE_SIGNAL_RE =
  /(?:переехал\p{L}*|переехал[аи]?|переехала|переезд|открыл(?:а|и)?\s+(?:новый\s+)?(?:уютный\s+)?(?:кабинет|офис|салон|студи\p{L}*|локаци\p{L}*|пространств\p{L}*)|открытие(?!\s+(?:сч[её]та|компан\p{L}*|виз\p{L}*|форм\p{L}*|анкет\p{L}*|грин[-\s]?карт\p{L}*|паспорт\p{L}*|банка|llc\b|инн\b|ein\b))|новый\s+(?:уютный\s+)?(?:кабинет|офис|салон|локаци\p{L}*)|новом\s+(?:уютном\s+)?(?:месте|кабинете|офисе|салоне)|отличные\s+новости|хорошие\s+новости|рады?\s+сообщить|теперь\s+я\s+работаю|теперь\s+работаю|в\s+честь\s+(?:этого|открытия)|special\s+offer\s+is\s+available|moved\s+to\s+(?:a\s+)?new|new\s+(?:location|office|studio|space)|grand\s+opening)/iu;
