-- Hotfix: listing_media DELETE must not depend on SELECT of parent listing
-- under FORCE RLS (SECURITY DEFINER without BYPASSRLS cannot see parent row).
-- Ownership for DELETE is derived from storage_path prefix listings/{auth.uid()}/...
...
begin
  if tg_op = 'DELETE' then
    ...
    -- Path-based ownership (works during ON DELETE CASCADE under FORCE RLS)
