-- Hotfix: elevated sessions (auth.uid() is null) may set profiles.created_at
-- for seeding / regression fixtures. Client sessions still cannot change it.
-- Restores reviews RLS suite compatibility after Pack 1 profile enforce trigger.
...
begin
  ...
  if tg_op = 'UPDATE' then
    ...
    -- Elevated SQL (auth.uid() is null) may update role and created_at for ops/seeding.
