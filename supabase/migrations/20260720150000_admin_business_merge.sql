-- Admin: read all businesses, set status, merge duplicates.
...
begin
  ...
  if p_status not in ('pending', 'approved', 'rejected', 'archived') then
...
-- Merge drop into keep: re-parent related rows, fill empty keep fields, archive drop.
...
revoke all on function public.admin_merge_businesses(uuid, uuid) from public, anon;
grant execute on function public.admin_merge_businesses(uuid, uuid) to authenticated;
