-- Public platform resource totals (businesses + listings + offers).
...
as $$
  ...
  all_resources as (
    select created_at from public_businesses
    union all
    select created_at from public_listings
    union all
    select created_at from public_offers
  )
