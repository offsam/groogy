-- Fix today/yesterday bounds for platform resource stats.
--
-- Bug: in this Postgres, `date AT TIME ZONE zone` casts the date to
-- timestamptz at UTC midnight first, then converts to local wall time
-- (timestamp without time zone). That made "today" start ~17:00 UTC
-- on the previous calendar day instead of local midnight.
--
-- Correct pattern: truncate the LA wall-clock timestamp, then interpret
-- that local midnight as America/Los_Angeles → timestamptz.
