-- Smarter enrich: also append unique queue description onto existing business text,
-- and allow re-enrich when the item is already linked to the same business.
...
begin
  ...
  -- Description strategy: fill if empty; otherwise append queue text when it adds
  -- meaningful new content (longer + not already contained).
  ...
  set
    ...
    image_url = case
      when nullif(btrim(coalesce(b.image_url, '')), '') is null then v_image else b.image_url
    end,
