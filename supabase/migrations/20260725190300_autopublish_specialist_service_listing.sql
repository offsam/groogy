-- Autopublish private specialists / services as Profi-style cards:
-- approved self-employed business (contacts) + public service listing (publisher=business).

drop policy if exists "service_role insert service_listing_details"
  on public.service_listing_details;
create policy "service_role insert service_listing_details"
  on public.service_listing_details
  for insert
  to service_role
  with check (true);

drop policy if exists "service_role select service_listing_details"
  on public.service_listing_details;
create policy "service_role select service_listing_details"
  on public.service_listing_details
  for select
  to service_role
  using (true);

drop policy if exists "service_role select listing_categories"
  on public.listing_categories;
create policy "service_role select listing_categories"
  on public.listing_categories
  for select
  to service_role
  using (true);

grant select on table public.listing_categories to service_role;
grant select, insert on table public.service_listing_details to service_role;
...
begin
  ...
  insert into public.businesses (
    ...
    coalesce(nullif(btrim(coalesce(p_city, '')), ''), 'Orange County'),
