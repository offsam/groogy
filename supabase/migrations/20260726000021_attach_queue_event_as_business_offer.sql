-- Attach import-review items (events / promos) as business_offers on an existing business.
-- Fill-empty enrich of the business is optional via service_enrich_business_from_queue separately.

drop policy if exists "service_role insert business_offers" on public.business_offers;
create policy "service_role insert business_offers"
  on public.business_offers
  for insert
  to service_role
  with check (true);

drop policy if exists "service_role select business_offers" on public.business_offers;
create policy "service_role select business_offers"
  on public.business_offers
  for select
  to service_role
  using (true);

drop policy if exists "service_role update business_offers" on public.business_offers;
create policy "service_role update business_offers"
  on public.business_offers
  for update
  to service_role
  using (true)
...
begin
  ...
  if v_item.review_status = 'approved' and v_item.published_entity_id is not null then
    return jsonb_build_object(
      ...
      'idempotent', true,
      'item_id', v_item.id,
      'business_id', v_item.published_entity_id,
  ...
  -- ASCII slug from title + short stamp
  ...
  insert into public.business_offers (
    ...
    'contact',
    'USD',
    -- offer_type=other only allows empty attributes (see business_offers_validate_attributes)
    '{}'::jsonb,
