-- Public social metrics for listing cards (not gated contacts).
...
comment on column public.businesses.yelp_rating is
  'Yelp star rating (1–5), safe to show on listing cards.';
comment on column public.businesses.yelp_reviews_count is
  'Yelp review count accompanying yelp_rating.';
comment on column public.businesses.instagram_followers_count is
  'Instagram follower count when known; safe to show on listing cards.';

-- Expose metrics on the anti-scrape public catalog view (no contact URLs).
