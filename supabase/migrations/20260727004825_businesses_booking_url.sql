-- Online booking / «Book now» URL scraped from the business website.

alter table public.businesses
  add column if not exists booking_url text;

comment on column public.businesses.booking_url is
  'Public online booking URL (Book Now / Calendly / Square Appointments, etc.).';

-- Rebuild anti-scrape public view: expose has_booking flag (not the raw URL)
-- so listing cards can show a Book chip without leaking the link until profile.
...
comment on view public.businesses_public is
  'Approved businesses without phone/email/URLs — safe for anon catalog. Includes city/ZIP and has_booking.';
