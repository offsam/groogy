-- Merge enrich: treat category SVG / placeholder images as empty so a real
-- queue photo can replace them; upgrade snake/junk business names when the
-- queue card has a richer identity.

create or replace function public.is_placeholder_image_url(p_url text)
returns boolean
language sql
immutable
as $$
  select
    p_url is null
    or btrim(p_url) = ''
    or p_url ilike '%/images/categories/%'
    or p_url ilike '%placeholder.svg';
$$;

create or replace function public.is_weak_entity_name(p_name text)
returns boolean
language plpgsql
immutable
as $$
declare
  v text := lower(btrim(coalesce(p_name, '')));
begin
  if v = '' then
    return true;
  end if;
  if v in (
    'messenger', 'whatsapp', 'telegram', 'instagram', 'facebook',
    'unknown', 'user', 'admin', 'null', 'none', 'n/a', 'без названия',
    'запись', 'reel', 'мастер', 'master', 'specialist', 'специалист'
  ) then
    return true;
  end if;
  -- snake_case / handle style: aigerim_bimoldina
  if v ~ '^[a-z0-9]+(_[a-z0-9]+)+$' then
    return true;
  end if;
  return false;
end;
...
revoke all on function public.is_placeholder_image_url(text) from public, anon, authenticated;
grant execute on function public.is_placeholder_image_url(text) to service_role;

revoke all on function public.is_weak_entity_name(text) from public, anon, authenticated;
grant execute on function public.is_weak_entity_name(text) to service_role;
