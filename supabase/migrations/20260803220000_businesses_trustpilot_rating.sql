-- Trustpilot URL + public TrustScore metrics (parallel to Yelp).
...
comment on column public.businesses.trustpilot_url is
  'Trustpilot review page URL (trustpilot.com/review/…).';
comment on column public.businesses.trustpilot_rating is
  'Trustpilot TrustScore (1–5), safe to show on listing cards.';
comment on column public.businesses.trustpilot_reviews_count is
  'Trustpilot review count accompanying trustpilot_rating.';

-- Rebuild anti-scrape public catalog view: metrics + has_trustpilot (no raw URL).
-- Restores has_booking which mention-count rebuild omitted.
...
comment on view public.businesses_public is
  'Approved businesses without phone/email/URLs — safe for anon catalog. Includes city/ZIP, community mentions, booking/trustpilot presence, and public ratings.';
