-- Facebook page recommend % + review count (from paste / directory, not star rating).

alter table public.businesses
  add column if not exists facebook_recommend_pct numeric,
  add column if not exists facebook_reviews_count integer not null default 0;

alter table public.businesses
  drop constraint if exists businesses_facebook_recommend_pct_chk;

alter table public.businesses
  add constraint businesses_facebook_recommend_pct_chk
  check (
    facebook_recommend_pct is null
    or (facebook_recommend_pct >= 0 and facebook_recommend_pct <= 100)
  );

alter table public.businesses
  drop constraint if exists businesses_facebook_reviews_count_chk;

alter table public.businesses
  add constraint businesses_facebook_reviews_count_chk
  check (facebook_reviews_count >= 0);

comment on column public.businesses.facebook_recommend_pct is
  'Facebook «X% recommend» (0–100), safe to show on listing cards.';
comment on column public.businesses.facebook_reviews_count is
  'Facebook recommendation / review count accompanying facebook_recommend_pct.';

-- Rebuild anti-scrape public catalog view with FB metrics.
drop view if exists public.businesses_public;

create view public.businesses_public
with (security_invoker = false)
as
select
  b.id,
  b.slug,
  b.category_id,
  b.name,
  b.short_description,
  b.description,
  b.status,
  b.rating_avg,
  b.reviews_count,
  b.ai_verified_reviews_count,
  b.transaction_verified_reviews_count,
  b.google_rating,
  b.google_reviews_count,
  b.yelp_rating,
  b.yelp_reviews_count,
  b.trustpilot_rating,
  b.trustpilot_reviews_count,
  b.facebook_recommend_pct,
  b.facebook_reviews_count,
  b.instagram_followers_count,
  b.image_url,
  b.city,
  b.region,
  b.state_code,
  b.postal_code,
  b.latitude,
  b.longitude,
  b.location_precision,
  b.opening_hours,
  b.created_at,
  b.updated_at,
  b.third_party_mention_count,
  (b.phone is not null and length(btrim(b.phone)) > 0) as has_phone,
  (b.email is not null and length(btrim(b.email)) > 0) as has_email,
  (
    b.website is not null
    and length(btrim(b.website)) > 0
    and b.website !~* 'instagram\.com|facebook\.com|fb\.com|yelp\.com|t\.me/|telegram\.me/'
  ) as has_website,
  (
    (b.instagram_url is not null and length(btrim(b.instagram_url)) > 0)
    or (b.website is not null and b.website ~* 'instagram\.com')
  ) as has_instagram,
  (
    b.telegram_url is not null
    and length(btrim(b.telegram_url)) > 0
  ) as has_telegram,
  (
    (b.yelp_url is not null and length(btrim(b.yelp_url)) > 0)
    or (b.website is not null and b.website ~* 'yelp\.com')
  ) as has_yelp,
  (
    (b.trustpilot_url is not null and length(btrim(b.trustpilot_url)) > 0)
    or (b.website is not null and b.website ~* 'trustpilot\.com')
  ) as has_trustpilot,
  (
    (b.website is not null and b.website ~* 'facebook\.com|fb\.com')
    or b.facebook_recommend_pct is not null
    or coalesce(b.facebook_reviews_count, 0) > 0
  ) as has_facebook,
  (
    (b.google_maps_url is not null and length(btrim(b.google_maps_url)) > 0)
    or (b.latitude is not null and b.longitude is not null)
  ) as has_google_maps,
  (
    coalesce(b.source_kind, '') = 'platform'
    or (
      b.source_url is not null
      and length(btrim(b.source_url)) > 0
      and coalesce(b.source_kind, '') <> 'platform'
    )
  ) as has_source,
  (
    b.booking_url is not null
    and length(btrim(b.booking_url)) > 0
  ) as has_booking
from public.businesses b
where b.status = 'approved';

comment on view public.businesses_public is
  'Approved businesses without phone/email/URLs — safe for anon catalog. Includes city/ZIP, community mentions, booking/trustpilot/facebook presence, and public ratings.';

grant select on public.businesses_public to anon, authenticated;

notify pgrst, 'reload schema';
